...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql = mysqli_query($connect, "select * from attendances where members_id='$id' and attendance='Early rain' and marked=1") or die('Failed to complete request');
    $result= mysqli_num_rows($sql);

    $sql2 = mysqli_query($connect, "select * from attendances where members_id='$id' and attendance='Latter rain' and marked=1") or die ('Failed to complete request');
