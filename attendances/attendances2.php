        ...
        <div class="page-wrapper">
                                            ...
                                            <div class="portlet box blue">
                                                    ...
                                                    <div class="portlet-body">
                                                        <table class="table table-striped table-bordered table-hover" id="sample_2">
                                                            ...
                                                            <tbody>
            <?php  $sql=mysqli_query($connect, "SELECT * FROM members WHERE deleted=0") or die("Unable to Process request");
                     if(!empty($sql)){
                        $j=0;
                        foreach($sql as $data){
                            $j++;
            ?>
                                                                <tr>
                                                                    ...
                                                                    <td> 
                                <?php 
                                $sql=mysqli_query($connect, "SELECT * FROM attendances WHERE members_id='$data[id]' OR created>='$time' AND attendance='Early rain'") or die("Unable to process request");
                                    $check=mysqli_fetch_assoc($sql);
                                    $mark=$check['marked'];
                                    if(mysqli_num_rows($sql)>0){
                                 ?>
                                                                        <div class="btn-group">
                                                                                    <a class="btn btn-xs blue" href="earlyrain.php?present=<?php echo $data['id'] ?>"  onclick='javascript:confirmationDelete($(this));return false;'>
                                                                                        <i class="icon-check"></i> PRESENT</a>
                                                                        </div> 
                                    <?php }else{ echo''; }?>
                                                                    </td>
                                                                     <td> 
                                <?php  $sql=mysqli_query($connect, "SELECT * FROM attendances WHERE members_id='$data[id]' AND created>='$time' AND attendance='Latter Rain'") or die("Unable to process request");
                                    $check=mysqli_fetch_assoc($sql);
                                    $mark=$check['marked'];
                                    if(mysqli_num_rows($sql)>0){
                                 ?>
                                                                        <div class="btn-group">
                                                                                    <a class="btn btn-xs blue" href="latterrain.php?present=<?php echo $data['id'] ?>"  onclick='javascript:confirmationDelete($(this));return false;'>
                                                                                        <i class="icon-check"></i> PRESENT</a>
                                                                        </div> 
                                    <?php }else{ echo''; }?>
                                                                    </td>
